//TESTCASE (Next Fit) 2: Running time random input of 100 floating point numbers in range [0.1-0.8]

import java.util.*;
import java.math.*;
import java.text.*;

public class testcaseNextFit2 {
	...
	public static void main(String args[]) {
		...
		testcaseNextFit2 nf2 = new testcaseNextFit2();
		int testSize = 100;
		...
		for(int i=1; i<=10; i++) {
			...
			System.out.println();
			nf2.data = binfunc.genNumbers(testSize);
			double numBins = binfunc.getBins(nf2.data);
			double dataSum = binfunc.getSum(nf2.data);
		...
		System.out.println();
		nf2.data = binfunc.readFile(filename);
		double numBins = binfunc.getBins(nf2.data);
		double dataSum = binfunc.getSum(nf2.data);
		double wasteFunc = binfunc.getWaste(numBins,dataSum);
		System.out.println("Input of 100 random floating point numbers in range [0.1-0.8]");
