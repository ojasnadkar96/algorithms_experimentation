//TESTCASE (First Fit) 4: Running time random input of 10000 floating point numbers in range [0.1-0.8]

import java.util.*;
import java.math.*;
import java.text.*;

public class testcaseFirstFit4 {
	...
	public static void main(String args[]) {
		...
		testcaseFirstFit4 ff4 = new testcaseFirstFit4();
		int testSize = 10000;
		String filename = "test10000.txt";
		ff4.data = binfunc.readFile(filename);
		...
		for(int i=1; i<=10; i++) {
			...
			System.out.println();
			ff4.data = binfunc.genNumbers(testSize);
			double numBins = binfunc.getBins(ff4.data);
			double dataSum = binfunc.getSum(ff4.data);
